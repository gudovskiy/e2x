...
class PredDiffAnalyser:
    ...
    def __init__(self, x, tar_func, sampler, C, true_index, num_samples=10, batch_size=10, prob_tar=True):
        '''
        Input:  
        ...
        fw = self.tar_func(self.x)  # true network state for the given input
        self.num_blobs = len(fw)
        ...
        fw = [t[0] for t in fw]
        true_tar_idx = range(C*true_index, C*(true_index+1))
        #print 'HEREEEEE',true_tar_idx
        self.true_tar_idx = true_tar_idx
        self.true_tar_val = [fw[0][true_tar_idx]]


#%%
# -------------------- METHOD RETURNING EXPLANATIONS --------------------------  
        
    def get_rel_vect(self, gt, gt_dims, win_size, overlap, stride):
        """
        ...
        """
        # for speed optimization process only part of the frame
        H,W = gt_dims
        #[xmin, ymin, xmax, ymax] = np.round( gt[5:9] * np.array([W,H,W,H]) ).astype(int)
        #mask = np.zeros((H,W))
        ...
        #print "rel_vects shape=",rel_vects[0].shape
        ...
        if overlap:
            ...
            win_idx = 0
            for i in range(0,self.x.shape[1]-win_size+1, stride): # rows
                #start_time = time.time()
                for j in range(0,self.x.shape[2]-win_size+1, stride): # columns
                    #if mask[i,j] == 1:
            ...
            # interpolate strided case
            if stride>1:
                for b in range(self.num_blobs):
                    p = rel_vects[b].reshape((H,W,-1))
                    pInterp = np.zeros(p.shape)
                    #print H,W,p.shape
                    for i in range(p.shape[-1]): # per class interpolation
                        f = interpolate.interp2d(np.arange(0,W,stride),np.arange(0,H,stride),p[::stride,::stride,i], kind='linear') # or 'cubic'
                        pInterp[:,:,i] = f(np.arange(0,W),np.arange(0,H))
                    #print p.shape, pInterp.shape
                    rel_vects[b] = pInterp.reshape((H*W,p.shape[-1])) # reshape back
        else:
            windows = np.zeros((self.tests_per_batch, win_size*win_size*3), dtype=int)
            win_idx = 0
...
#        for f in xrange(feature_sets.shape[0]):
        for f in xrange(len(feature_sets)):
            ...
            #print x_new[f, :, feature_sets[f].ravel()].shape, np.mean(x_new[f, :, feature_sets[f].ravel()])
        ...
        for b in xrange(self.num_blobs):
            ...
            tarValsD.append(tarVals[b][:,:, self.true_tar_idx])

        # evaluate the prediction difference
        rel_vect = self._evaluate_probability_difference(tarValsD)
...
# -------------- EVALUATION OF THE PREDICTION DIFFERENCE ----------------------        
    ...
    def _evaluate_weights_evidence(self, tarVals):
        '''
    ...
    def _evaluate_probability_difference(self, tarVals):
        '''
        Evaluating the prediction difference: [-1.0:1.0]
        Input:  tarVals     the values of all the blobs for several altered inputs
                            length is self.num_blobs, dimensions of each blob 
                            are (num_featsures)*(shape of blob)
        '''
