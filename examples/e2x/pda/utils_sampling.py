# -*- coding: utf-8 -*-
"""
Classes for (1) marginal and (2) conditional sampling for data.
...
class marg_sampler:
    ...
    def get_samples(self, sampleIndices, featVect=None, numSamples=1):
        '''
        ...
        '''
        # we replace patch to gray rectangle!!!
        #return 128.0 * np.ones((1,len(sampleIndices))) # self.X[:numSamples], sampleIndices.ravel(), axis=1)
        return np.zeros((1,len(sampleIndices)))

class cond_sampler:
    '''
    Conditional sampler for image patches using a multivariate Gaussian distribution
    '''
    
    def __init__(self, win_size, padding_size, image_dims, dataset, net, netname, folder_name, folder_est_name, num_samples_fit=20000):
        '''
        ...
        Input: 
                ...
                image_dim       the (2d) dimensions of the image, i.e.,
                                (width, height)
                datset          dataset to use
                net            the convnet which is being used
                                (needed because each net preprocesses the data differently)
        ...
        self.folder_est_name = folder_est_name
        self.num_samples_fit = num_samples_fit

        self.path_folder = '/tmp/pda/gaussians/'
        ...
        self.meanVects, self.covMats = self._get_gauss_params(net)
        
        # the min/max values for the features seen in the data, so that we can cut off overfloating values
        if not os.path.exists(self.path_folder+'{}_{}_minMaxVals.npy'.format(self.dataset, self.netname)):
            self._save_minmax_values(net)

        self.minMaxVals = np.load(self.path_folder+'{}_{}_minMaxVals.npy'.format(self.dataset, self.netname))
    ...
    def _get_gauss_params(self, net):
        ...
        path_mean = self.path_folder+'{}_{}_means{}_indep'.format(self.dataset, self.netname, self.patchSize)
        path_cov  = self.path_folder+'{}_{}_covs{}_indep'.format(self.dataset, self.netname, self.patchSize)
    ...
    def _save_minmax_values(self, net):
        ...
        if not os.path.exists(self.path_folder):
            os.makedirs(self.path_folder)
        np.save(self.path_folder+'{}_{}_minMaxVals'.format(self.dataset, self.netname), minMaxVals)   
    #
    ...
    #        samples = np.random.multivariate_normal(cond_mean, cond_cov, numSamples)
        ...
        return samples
#
def estSampling(im, mean, win_size, stride, overlap):
    #
    sampling = 'marginal'
    cTH = 1e-4
    #print im.shape, win_size, stride, overlap
    # per-detection estimate
    #x = np.reshape(im,(im.shape[0],im.shape[1]*im.shape[2])) # make matrix
    #m = np.tril(np.corrcoef(x)-np.ones((im.shape[0],im.shape[0])))
    #c = np.sum(np.abs(m)) # calc average of Pearson coeffs with subtracted ones
    #if c < cTH: # if average correlation is higher than threshold, use conditional
    #    sampling = 'conditional'
    # per-patch estimate
    x = np.zeros((im.shape[0],win_size**2))
    if overlap:
        for i in range(0,im.shape[1]-win_size+1, stride): # rows
            for j in range(0,im.shape[2]-win_size+1, stride): # columns
                x = np.reshape(im[:,i:i+win_size,j:j+win_size],(im.shape[0],win_size**2)) # make matrix
                m = np.tril(np.corrcoef(x)-np.ones((im.shape[0],im.shape[0])))
                c = np.sum(np.abs(m)) # calc average of Pearson coeffs with subtracted ones
                #print c
                if c < cTH: # if average correlation is higher than threshold, use conditional
        ...
        for i in range(im.shape[1]/win_size): # rows
            for j in range(im.shape[2]/win_size): # columns
                x = np.reshape(im[:,i:i+win_size,j:j+win_size],(im.shape[0],win_size**2)) # make matrix
                m = np.tril(np.corrcoef(x)-np.ones((im.shape[0],im.shape[0])))
                c = np.sum(np.abs(m)) # calc average of Pearson coeffs with subtracted ones
                if c < cTH: # if average correlation is higher than threshold, use conditional
