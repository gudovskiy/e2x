...
void Solver<Dtype>::TestDetection(const int test_net_id) {
  ...
  for (int i = 0; i < param_.test_iter(test_net_id); ++i) {
    ...
    for (int j = 0; j < result.size(); ++j) {
      CHECK_EQ(result[j]->width(), EVAL_SHAPE);
      const Dtype* result_vec = result[j]->cpu_data();
      int num_det = result[j]->height();
      for (int k = 0; k < num_det; ++k) {
        int item_id = static_cast<int>(result_vec[k * EVAL_SHAPE]);
        int label = static_cast<int>(result_vec[k * EVAL_SHAPE + 1]);
        if (item_id == -1) {
          // Special row of storing number of positives for a label.
          if (all_num_pos[j].find(label) == all_num_pos[j].end()) {
            all_num_pos[j][label] = static_cast<int>(result_vec[k * EVAL_SHAPE + 2]);
          } else {
            all_num_pos[j][label] += static_cast<int>(result_vec[k * EVAL_SHAPE + 2]);
          }
        } else {
          // Normal row storing detection status.
          float score = result_vec[k * EVAL_SHAPE + 2];
          int tp = static_cast<int>(result_vec[k * EVAL_SHAPE + 3]);
          int fp = static_cast<int>(result_vec[k * EVAL_SHAPE + 4]);
          if (tp == 0 && fp == 0) {
